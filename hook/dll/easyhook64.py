...
class Hook:
    ...
    dll = ctypes.cdll.LoadLibrary(ctypes.util.find_library(str(pathlib.Path(__file__).parent / "EasyHook64.dll")))
    lh_install_hook = _win_api(
        dll.LhInstallHook, ctypes.c_ulong, (ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p)
    )
    ...
    lh_set_inclusive_acl = _win_api(
        dll.LhSetInclusiveACL, ctypes.c_ulong, (ctypes.c_void_p, ctypes.c_ulong, ctypes.c_void_p)
    )
    lh_set_exclusive_acl = _win_api(
        dll.LhSetExclusiveACL, ctypes.c_ulong, (ctypes.c_void_p, ctypes.c_ulong, ctypes.c_void_p)
    )
    ...
    def __init__(self, at: int, hook_func, restype: Any = ctypes.c_void_p, argtypes=()):
        self.at = at
        ...
        self.hook_info = HOOK_TRACE_INFO()
