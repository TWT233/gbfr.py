...
from .pipe import PipeClient, PipeServer, PipeServerHandler
from .base import _T
...
class RpcClient(PipeClient):
    class ResEventList(Generic[_T]):
        class ResEvent(threading.Event, Generic[_T]):
            def __init__(self):
                super().__init__()
                self.res = None
                self.is_exc = False
    ...
    reply_map: Dict[int, ResEventList]
    ...
    def res_iterator(self, reply_id, evt_list, first_res):
        try:
            ...
            while True:
                ...
                if reply_type == _Rpc.RETURN_EXCEPTION:
                    raise _Rpc.set_exc(*res)
                if reply_type == _Rpc.RETURN_GENERATOR_END:
                    break
                yield res
        finally:
